...
Prototype, Zero-to-one reference prototype demonstrating the proposed architecture and security boundaries. Not production: sessions are demo tokens without passwords, the rule planner covers the example vocabulary, and the local embedder is a stand-in for a real model. The architecture — tenant derivation, allow-listed queries, scoped semantic retrieval, independent citations, chained audit — is the part intended to survive into production unchanged.
